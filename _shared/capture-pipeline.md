...
Supported image types are `.png`, `.jpg`, `.jpeg`, `.webp`, `.gif`. Input validation happens before any vision-LLM call or attachment move:

- If a user passes a file path argument that exists but does not match a supported extension, reject with:
  ```
  Unsupported input type: <ext>. /braindump and capture skills accept text, markdown, and image inputs.
  ```
- If a path argument is missing, unreadable, or not a regular file:
  ```
  Image not found or unreadable: <path>
  ```

No vision-LLM call or file move occurs in either error case.

### Attachment directory

Before moving any images:

1. Resolve `<vault_root>` per §1.
2. Ensure `<vault_root>/_attachments/` exists. Create it silently if absent.

### Image-to-note naming (all skills)

- **New note from images:** filename is `<note-slug>.<ext>` (primary image). Additional images in the same note use collision suffix: `<note-slug>-2.png`, `<note-slug>-3.png`, etc.
- **Daily note image:** filename is `<YYYY-MM-DD>-<vision-slug>.<ext>` where `vision-slug` is generated by the vision-LLM (≤40 chars, slug-format). Collisions: `<YYYY-MM-DD>-<vision-slug>-2.ext`, etc.
- **Braindump unassigned images:** fallback filename is `<date>-braindump-unassigned-N.<ext>` (see braindump rules below).

All images are moved (not copied) from source to `<vault_root>/_attachments/`.

### Embed syntax (all skills)

Images are embedded in note body via Obsidian embed syntax:
```
![[filename.png]]
```

For `/daily`: embed is indented two spaces on the next line within the bullet:
```
- HH:MM description here
  ![[YYYY-MM-DD-vision-slug.png]]
```

For `/note` and `/braindump`: embeds appear at the end of the body, after the vision-LLM description, in input order.

### Frontmatter `attachments` field (note and braindump only)

When a note is created or updated with images, add (or extend) an `attachments:` list in frontmatter:

```yaml
attachments: ["filename1.png", "filename2.png"]
```

`/daily` bullets do not include an `attachments` field in frontmatter — they are append-only logs, not structured knowledge objects.

### Vision-LLM processing (image-input contract)

When an image argument is detected:

1. **Image validation:** Check path existence and supported extension. Abort on error (see "Supported image types and validation" above).
2. **Vision-LLM call:** Single LLM call includes all images + any text argument in the user's input.
3. **LLM output contract:**
   - For `/note` and `/braindump`: title (≤80 chars), topic (if applicable), description (verbatim OCR + scene), tags (if applicable).
   - For `/daily`: description (concise, ≤140 chars preferred), vision-slug (≤40 chars, slug-format).
   - On failure: abort with `Vision processing failed: <reason>. Image not moved, note not created.` Never move files if vision processing fails.

### URL detection and redirect (notes only)

When the user invokes `/note <arg>` and `<arg>` starts with `http://` or `https://`:

1. Prompt the user exactly once: `Detected URL: <url>. Ingest via /ingest? [y/n]`
2. Wait for single-character response.
3. If `y`: invoke the `/ingest` skill with the URL. On success, confirm: `Ingested via /ingest: <wiki-page>`. Do not create a note.
4. If `n`: proceed to standard CAPTURE flow, treating the URL as plain verbatim text.

`/braindump` and `/daily` receive URL args → NO URL prompt; URLs captured verbatim as text.

### MATCH path with images (notes and braindump)

When MATCH is decided and the new input includes images:

1. Generate a new vision-LLM description (same contract as for NEW).
2. Append the new description after the `---` separator (per existing MATCH shape).
3. Move images to `_attachments/` with the **existing note's slug** + collision-suffixed indices.
4. Extend the existing note's `attachments:` list with the new image filenames.
5. Do not recompute the note's title — only append the new content and attachment list.
